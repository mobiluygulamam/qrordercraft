@if(count($menus))
    <div class="row">
        @foreach($menus as $key => $menu)
            ...
            @php
            $mImg = "";
            $lazyloadsrc="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC";
            if (str_contains($menu->image, "unsplash") !== false) {
                $mImg = $menu->image;
                $lazyloadsrc = $menu->image;
            } else {
        ...
        @endphp
            <div
                ...
                data-description="{{$menu->description}}" data-image-url="{{$mImg}}"
                ...
                <div class="menu_item">
                    <figure>
                        ...
                        <img class="lazy-load"
                             src="{{$lazyloadsrc}}"
                             data-original="{{$mImg}}" alt="{{$menu->name}}">
                    </figure>
