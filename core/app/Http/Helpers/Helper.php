 ...
 * @return false|mixed
 */
function getTablesByRestoId($id, $key = null, $default = null)
{
     $table = BusinessTable::where('restoid', $id)->orderBy('table_id','asc')->get();
      // Alınan verileri uygun şekilde işleyerek JSON olanları decode et
      foreach ($table as $key => $value) {
          $value = Str::isJson($value) ? json_decode($value) : $value;
          $table[$key] = $value;
      }
      Log::info($table);
      // Dönüştürülmüş seçenekleri obje olarak döndür
      return array_to_object($table->toArray());
}
 ...
 * @return string
 */
function coffecraft_xor($str, $password = '')
{
    $len = strlen($str);
    $gamma = '';
    $n = $len > 100 ? 8 : 2;
 ...
 * @return string
 */
function coffecraft_xor_encrypt($str, $password = '')
{
    return base64_encode(coffecraft_xor($str, $password));
}
 ...
 * @return string
 */
function coffecraft_xor_decrypt($str, $password = '')
{
    return coffecraft_xor(base64_decode($str), $password);
}
